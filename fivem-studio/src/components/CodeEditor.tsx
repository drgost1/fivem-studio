...
export default function CodeEditor({
  ...
  const editorRef = useRef<import("monaco-editor/editor").editor.IStandaloneCodeEditor | null>(null);
  const editorDisposablesRef = useRef<import("monaco-editor/editor").IDisposable[]>([]);
  ...
  const handleValidate = useCallback((markers: import("monaco-editor/editor").editor.IMarker[]) => {
  ...
  useEffect(() => {
    ...
    for (const previous of trackedPathsRef.current) {
      if (!current.has(previous)) monacoInstance.editor.getModel(monacoInstance.Uri.parse(previous))?.dispose();
    }
  ...
  useEffect(() => {
    return () => {
      ...
      for (const path of trackedPathsRef.current) {
        monacoInstance.editor.getModel(monacoInstance.Uri.parse(path))?.dispose();
      }
    };
  }, [monacoInstance]);
